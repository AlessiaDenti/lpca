...
def main(gpu, arch, out_dir, dataset, train_dir, val_dir, warmUpIter, lr, lrg, eta, nbEpoch, batchsize, vgg_dropout, \
        momentum=0.9, weightDecay = 5e-4, lrSchedule = [200, 300], num_pc=50, mu=0, nested=1.0, mixup = 0, resumePth=None,  freeze_bn=False, pretrained=False):
    ...
    #    net.cuda()
    net = vgg.VGG(h_dim=num_pc, num_classes=nb_cls, arch = arch, pretrained = pretrained, dataset = dataset, vgg_dropout = vgg_dropout)
    net.cuda()
